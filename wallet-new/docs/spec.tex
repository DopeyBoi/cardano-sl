...
\end{equation*}
%
and by Lemma~\ref{lem:utxo_props_subtractdom_dom}
%
\begin{equation*}
...
\emph{change} to increase the balance. In this section we therefore extend the
wallet's model to keep track of ``expected UTxO''---the outputs that we expect
to become part of the wallet's UTxO---to clarify this situation.

\subsection{Model}

We extend the basic model of the wallet with rollback from
Figure~\ref{fig:basic_rollback_model} to additionally include the \emph{expected
UTxO}: the outputs the wallet expects to become available in its UTxO. The new
model is shown in Figure~\ref{fig:model_with_expected_UTxO}.
...
wallet's UTxO now get added to the expected UTxO. Vice versa, when the
wallet applies a block, any confirmed outputs are removed from expected.
...
\begin{proof}[Proof (sketch)]
Similar to the proof of Lemma~\ref{lem:bounds_in_basic_model}. The lower bound
is trivial, and the upper bound follows from
Invariant~\ref{inv:txins_in_dom_utxo_expected_pending}.
\end{proof}

Although we additionally trivially have that
%
\begin{equation*}
...
\begin{equation*}
\mathsf{txins} ~ \mathit{pending} \mathbin{\slashed\subseteq} \dom (\mathit{utxo} \cup \mathsf{change} ~ \mathit{pending})
\end{equation*}
(as discussed at the start of Section~\ref{sec:tracking_expected_UTxO}).
Nonetheless, we would like to establish a bound on the wallet's balance in
between
\begin{math}
\mathsf{availableBalance} ~ (\mathit{utxo}, \mathit{pending})
\end{math},
which removes all inputs spent by pending transactions but does not add back
any of the change, and
\begin{math}
\mathsf{totalBalance} ~ (\mathit{utxo} \cup \mathit{expected}, \mathit{pending})
\end{math},
which assumes that the expected unspent outputs will all become available.
...
the model with rollback. When a pending transaction depends on an expected
output, it can never happen that that pending transaction is confirmed but the
expected incoming transaction is not. Let's refer to these subsets of confirmed
pending and expected transactions as ``possible futures''. Then a reasonable
question we might ask is: what is the wallet's \emph{minimum balance across all
possible futures}?
...
Since we only keep track of the expected UTxO, and therefore miss dependency
information, we cannot determine if two expected transactions can both be
confirmed in the blockchain. We will therefore conservatively\footnote{ Note
that the absence of some dependency information only gives us \emph{more}
...
will still be a lower bound.} estimate the minimum balance as
%
...
\todo[inline]{How does this relate to the bounds we started with in this section?}
...
\begin{equation*}
  ...
  \right|_{\mathrlap{\mathsf{txins} ~ p \subseteq \dom(\mathit{utxo} \cup e \cup \mathsf{change} ~ p)}}
  ~ \mathsf{totalBalance} ~ (\mathit{utxo} \cup e, p)
\end{equation*}
%
